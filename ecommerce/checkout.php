...
// Ambil nama pengguna dan email dari tabel users
$get_user_data = $conn->prepare("SELECT name, email FROM users WHERE id = ?");
$get_user_data->execute([$user_id]);

if ($get_user_data->rowCount() > 0) {
   $user_data = $get_user_data->fetch(PDO::FETCH_ASSOC);
   $user_name = $user_data['name'];
   $email = $user_data['email']; // Ambil email dari tabel users
} else {
   $user_name = ''; // Default jika nama tidak ditemukan
   $email = ''; // Default jika email tidak ditemukan
}
...
<body>
   ...
   <section class="checkout-orders">
      <form action="" method="POST">
            ...
            <h3>Masukkan Pesanan Anda</h3>
            <div class="flex">
               <div class="inputBox">
               <span>Nama Anda:</span>
               <input type="text" name="name" placeholder="Masukkan nama Anda" class="box" maxlength="20" value="<?= $user_name; ?>" required>
            </div>
